...
def scrape_data():
    ...
    while i < n_products:
        ...
        wait = 0
        i += 1
        current_product_data = pd.DataFrame(current_product.__dict__, index = [0])
        df_product_data = pd.concat([df_product_data, current_product_data])

    t_end = time.perf_counter()
    print(f'Time elapsed: {t_end - t_start}')

    return df_product_data